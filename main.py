...
async def sayHello(interaction: discord.Interaction):
    ...
    try:
        ...
        director = '0/400.000.000'
        terrorist = '123.456.789/400.000.000'
        beast = '200.000.000/400.000.000'
        knight = '380.000.000/400.000.000'
        level = 'Lv. 105'

        cards = []
        cards.append(gen_card_from_ocr(director, level, 0, boss_df))
        cards.append(gen_card_from_ocr(terrorist, level, 1, boss_df))
        cards.append(gen_card_from_ocr(knight, level, 3, boss_df))
        cards.append(gen_card_from_ocr(beast, level, 2, boss_df))
        ...
        print(e)
        car = discord.File('assets/big_ass_card.jpg')
        mtime = os.path.getmtime('assets/big_ass_card.jpg')
        ctime = time.ctime(mtime)
        embed = discord.Embed(title="Update that bai", description="Gui lai kq gan nhat: " + ctime)
        embed.set_image(url='attachment://big_ass_card.jpg')
        await interaction.followup.send(embed=embed, file=car)
